...
seir_model <- si_age$new(pars = list(m = ref_m, d = 1/ref_d, a = a, 
                                     ...
                                     import = import, 
                                     N_time = dim(import)[3], N_age = nrow(ref_m), N_reg = nrow(ref_d), 
...
states <- c("S", "V1", "V2", "Es", "Ev1", "Ev2", "Is", "Iv1", "Iv2", "R")
...
## Compute the number of cases per region / age group across simulations
print(apply(final_res[grep("R_", rownames(final_res)), ] - c(R), 1, summary))
## Compute the total number of cases per region across simulations
print(summary(colSums(final_res[grep("R_", rownames(final_res)), ] - c(R))))
## Compute the total number of single vaccinated cases per region across simulations
print(summary(colSums(c(V1) - final_res[grep("V1_", rownames(final_res)), ])))
## Compute the total number of double vaccinated cases per region across simulations
print(summary(colSums(c(V2) - final_res[grep("V2_", rownames(final_res)), ])))



### Generate plot of the number of Susceptibles, Infected and Recovered individuals through time
### per region / age group
png(filename = "Output/fig_sir_import.png", width = 600, height = 800)
...
for(i in seq_len(nrow(ref_d))){
  for(j in seq_len(nrow(ref_m))){
    ...
    recovered <- output[c(paste0("R", lab_i_j)),, ]
...
png(filename = "Output/fig_new_cases_import.png", width = 600, height = 800)
par(mfrow = c(nrow(ref_d),nrow(ref_m)), mar = c(3, 4, 2, 0.5), mar = c(3, 4, 1, 0), 
    oma = c(2, 2, 0, 2), las = 1, bty = "l")
...
for(i in seq_len(nrow(ref_d))){
  for(j in seq_len(nrow(ref_m))){
    ...
    lab_i_j <- c(paste0("Is_reg", i, "_age", j), paste0("Iv1_reg", i, "_age", j),
                 paste0("Iv2_reg", i, "_age", j))
